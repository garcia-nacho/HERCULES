...
ptl2<- ggplotly(ggplot(lineages.clean)+
...
ggplot(lineages.clean)+
  ...
  scale_colour_manual(values = rainbow(length(unique(lineages.clean$Recompressed))))+
  theme_minimal()+
  facet_trelliscope( ~Date, as_plotly = TRUE, path = "/Data/results/analysis/Widgets", self_contained = FALSE)

file.rename("/Data/results/analysis/Widgets/index.html","/Data/results/analysis/Widgets/LineageMap_Trelliscope.html" )


weeks<-unique(lineages.clean$Week)
...
for (i in 1:nrow(lineages.clean)) {
  muts<-unlist(base::strsplit(lineages.clean$CleanMut.aa[i],"/"))
  if(muts[1]!="None") muts<-muts[order(as.numeric(gsub("S:.","", gsub(".$","",muts))))]  
  if(length(grep("X",muts))>0) muts<-muts[-grep("X", muts)]
  lineages.clean$CleanMut.aa[i]<-paste(muts, collapse = "/")
}
if(length(which(duplicated(lineages.clean$Mut.aa)))>0){
  uniqueaa<-lineages.clean[-which(duplicated(lineages.clean$Mut.aa)),]  
}else{
  uniqueaa<-lineages.clean
}
...
voc.agg<-aggregate(Ratio~Seq, lineages.clean, max)
colnames(voc.agg)[2]<-"MaximumRatio"
voc.agg<-voc.agg[which(voc.agg$MaximumRatio>ratio.co),]
voc.agg<-merge(voc.agg, lineages.clean[-which(duplicated(lineages.clean$Seq)),c("Mut.aa", "CleanMut.aa","PangoRecompressed","Seq")],by="Seq")

lineageswwmtx<-strsplit(voc.agg$Seq,"")
lineageswwmtx<-base::do.call(rbind, lineageswwmtx)

distmat<-as.data.frame(matrix(NA, ncol = nrow(lineageswwmtx), nrow = nrow(lineageswwmtx)))
pb<-txtProgressBar(max = nrow(lineageswwmtx))
for(i in 1:nrow(lineageswwmtx)) {
  setTxtProgressBar(pb,i)
  for(j in 1:nrow(lineageswwmtx)) {
    distmat[i,j] <- 1-length(which (lineageswwmtx[i,] == lineageswwmtx[j,]))/ncol(lineageswwmtx) 
    
  }  
  
}

colnames(distmat)<-voc.agg$Seq
...
graph.undir<-merge(graph.undir, voc.agg , by.x="name", by.y="Seq")



ptl4<- ggplotly(ggplot(graph.undir, aes(x = x, y = y, xend = xend, yend = yend)) +
                  geom_edges(color = "black", alpha = 0.2, curvature = 0) +
                  geom_nodes( aes(size=MaximumRatio, col=PangoRecompressed , text=CleanMut.aa ), alpha=0.3)+
                  #  geom_text(data=labs,aes(xend, yend, label=sample))+
                  theme_minimal(), tooltip = c("PangoRecompressed", "MaximumRatio","CleanMut.aa") )


saveWidget(partial_bundle(ptl4), "TotalMST.html")

graph.undir$MaxDate <-NA 
for (i in 1:nrow(graph.undir)) {
graph.undir$MaxDate[i]<-as.character(lineages.clean$Date[which(lineages.clean$CleanMut.aa== graph.undir$CleanMut.aa[i] &
                                                    lineages.clean$Ratio==graph.undir$MaximumRatio[i])][1])
    
}
graph.undir$MaxDate<-as.Date(graph.undir$MaxDate)
graph.undir$MaxDatetxt<-as.character(graph.undir$MaxDate)

ptl4<- ggplotly(ggplot(graph.undir, aes(x = x, y = y, xend = xend, yend = yend)) +
                  geom_edges(color = "black", alpha = 0.2, curvature = 0) +
                  geom_nodes( aes(size=MaximumRatio, col=MaxDate , text=CleanMut.aa, label=MaxDatetxt ), alpha=0.5)+
                  #  geom_text(data=labs,aes(xend, yend, label=sample))+
                  scale_color_gradient(low = "red", high = "blue",
                                       limits=as.Date(c(min(graph.undir$MaxDate), max(graph.undir$MaxDate)), origin = as.Date(min(graph.undir$MaxDate))),
                                       trans="date")+
                  theme_minimal(), tooltip = c("MaximumRatio", "CleanMut.aa", "MaxDatetxt") )

saveWidget(partial_bundle(ptl4), "TotalMST_Date.html")

write_xlsx(graph.undir, "MSTdata.xlsx")

voc.agg2<-merge(graph.undir, lineages.clean[,c("Seq","Ratio","Count","Location","Week","Date","Year","Experiment")], by.x = "name", by.y="Seq")

dates<-unique(voc.agg2$Date)
`%!in%` = Negate(`%in%`)
for (i in 1:length(dates)) {
  dummy<-voc.agg2[which(voc.agg2$Date==dates[i]),]
  if(length(which(dummy$xend %!in% dummy$x))> 0)  dummy<-dummy[-which(dummy$xend %!in% dummy$x ),]
  if(length(which(dummy$xend %!in% dummy$x))> 0) dummy<-dummy[-which(dummy$yend %!in% dummy$y ),]
  if(!exists("voc.agg.clean")){
    voc.agg.clean<-dummy
  }else{
    voc.agg.clean<-rbind(voc.agg.clean, dummy)
  }
}

ptl2<- ggplotly(ggplot(voc.agg.clean, aes(x = x, y = y, xend = xend, yend = yend)) +
                  geom_segment(color = "black", alpha = 0.6) +
                  geom_nodes(aes(size=Ratio, text=CleanMut.aa, col=PangoRecompressed), alpha=0.2)+
                  scale_color_manual(values = rainbow(length(unique(voc.agg.clean$PangoRecompressed))))+
                  #geom_text(data=labs,aes(xend, yend, label=sample))+
                  theme_minimal()+
                  xlab("MST X coordinate")+
                  ylab("MST Y coordinate")+
                  facet_wrap( ~as.Date(Date)), tooltip = c("Ratio", "CleanMut.aa", "PangoRecompressed"))

saveWidget(partial_bundle(ptl2), "MST_TotalFacet.html")

lin.to.pad<-unique(voc.agg.clean$PangoRecompressed)
dates<-unique(voc.agg.clean$Date)

for (d in 1:length(dates)) {
  lin.present<-unique(voc.agg.clean$PangoRecompressed[which(voc.agg.clean$Date==dates[d])])
  lin.missing<-lin.to.pad[-which(lin.present %in% lin.to.pad)]
  if(length(lin.missing)>0){
    
    padd<-voc.agg.clean[1,,drop=FALSE]
    padd[1,]<-NA
    padd<-padd[rep(1,length(lin.missing)),]
    padd$PangoRecompressed<-lin.missing
    padd$Date<-as.Date(dates[d])
    voc.agg.clean<-rbind(voc.agg.clean,padd)
    
  }
}


ggplot(voc.agg.clean, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_segment(color = "black", alpha = 0.6) +
  geom_nodes(aes(size=Ratio, text=CleanMut.aa, col=PangoRecompressed), alpha=0.2)+
  scale_color_manual(values = rainbow(length(unique(voc.agg.clean$PangoRecompressed))))+
  #geom_text(data=labs,aes(xend, yend, label=sample))+
...
file.rename("/Data/results/analysis/Widgets/index.html","/Data/results/analysis/Widgets/MST_Trelliscope.html" )

if(length(which(is.na(voc.agg.clean$name)))>0) voc.agg.clean<-voc.agg.clean[-which(is.na(voc.agg.clean$name)),]
write_xlsx(voc.agg.clean, "MSTdata_aggregated_date.xlsx")
...
lineages.agg$Date<-as.Date(gsub(".*\\.","",gsub("_.*","",lineages.agg$Sample)))
lineages.agg$Experiment<-gsub("\\..*","",lineages.agg$Sample)
lineages.agg$Experiment<-toupper(lineages.agg$Experiment)
lineages.agg$Year<-year(lineages.agg$Date)
lineages.agg$Week<-week(lineages.agg$Date)

if(length(which(duplicated(lineages.clean$Mut.aa)))>0) uniqueaa<-lineages.clean[-which(duplicated(lineages.clean$Mut.aa)),]
...
voc.agg<-aggregate(Count~CleanMut.aa+Date, lineages.clean, sum )
...
voc.agg.count$Count <- length(unique(date.agg$Date))

voc.agg.count$Totrack <- voc.agg.count$Ratio/voc.agg.count$Count

if(length(which(voc.agg.count$Totrack>0.1))){
  lineages.to.follow<-intersect(lineages.to.follow, voc.agg.count$CleanMut.aa[which(voc.agg.count$Totrack>0.1)])  
}
  

if(length(lineages.to.follow)>0){
...
ggsave("MutationLineageLinePlot.pdf" ,width = 20, height = 16)
...
if(length(unique(Mutation.df.agg$Sample))<=10){
  ...
  ggsave("SingleMutationsBarplots.pdf" ,width =  8.27, height =  11.69)
}
